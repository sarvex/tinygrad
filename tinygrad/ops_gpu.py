...
class ReLU(Function):
  def forward(self, input):
    ...
    return unary_op(self, 'max(a, (float)0.)', input)

  def backward(self, grad_output):
    input, = self.saved_tensors
    return binary_op(self, 'a * (b >= 0)', grad_output, input)

class Log(Function):
  def forward(self, input):
    self.save_for_backward(input)
    return unary_op(self, 'log(a)', input)

  def backward(self, grad_output):
    input, = self.saved_tensors
    return binary_op(self, 'a / b', grad_output, input)

class Exp(Function):
  def forward(self, input):
    ret = unary_op(self, 'exp(a)', input)
    self.save_for_backward(ret)
    return ret

  def backward(self, grad_output):
    ret, = self.saved_tensors
    return binary_op(self, 'a * b', grad_output, ret)
...
class Sum(Function):
  def forward(self, input, axis=None):
    ...
    ret = reduce_op(self, "out += a", "out", input, axis=axis)
    if axis is not None:
      ret.shape = tuple(input.shape[i] for i in range(len(input.shape))
                        if i not in axis)
    return ret
  ...
  def backward(self, grad_output):
    ...
    return binary_op(self, 'a+b', output, buffer_new(self, input.shape, zero=True))
...
class Max(Function):
  ...
  def backward(self, grad_output):
    ...
    shape = [1 if axis is None or i in axis else input.shape[i] for i in range(len(input.shape))]
    ret2 = binary_op(self, "1.0*(a==b)", input, GPUBuffer(shape, ret))
    div = reduce_op(self, "out += a", "out+1e-10", ret2, axis=axis)
    ret3 = binary_op(self, "a/b", ret2, GPUBuffer(shape, div))
    return binary_op(self, 'a*b', ret3, GPUBuffer(shape, grad_output))
...
def get_binop_prg(cl_ctx, code, complist):
  ...
  return cl.Program(
      cl_ctx,
      ((((((((
          (f"""__kernel void binop(__global const float *x_g, __global const float *y_g, __global float *res_g{args}"""
           + """) {
...
def binary_op(ctx, code, x, y):
  ...
  prg.binop(ctx.cl_queue, [prod_list[0]] if dimlist else [1], None, x.cl, y.cl,
            ret.cl, *dimlist, *(prod_list[1:]))
  return ret
...
class Add(Function):
  def forward(self, x, y):
    ...
    return binary_op(self, 'a+b', x, y)

  def backward(self, grad_output):
    grad_x, grad_y = grad_output, grad_output
    shape_x, shape_y = self.saved_tensors
    return unbroadcast(self, grad_x, shape_x), unbroadcast(self, grad_y, shape_y)
...
class Sub(Function):
  ...
  def backward(self, grad_output):
    grad_x, grad_y = grad_output, unary_op(self, '-a', grad_output)
    shape_x, shape_y = self.saved_tensors
    return unbroadcast(self, grad_x, shape_x), unbroadcast(self, grad_y, shape_y)
...
class Mul(Function):
  ...
  def backward(self, grad_output):
    ...
    grad_x = binary_op(self, 'a*b', y, grad_output)
    grad_y = binary_op(self, 'a*b', x, grad_output)
    return unbroadcast(self, grad_x, x.shape), unbroadcast(self, grad_y, y.shape)

class Pow(Function):
  def forward(self, x, y):
    self.save_for_backward(x, y)
    return binary_op(self, 'pow(a,b)', x, y)
  ...
  def backward(self, grad_output):
    ...
    grad_x = binary_op(
        ...
        'a*b',
        grad_output,
        binary_op(self, 'b * (pow((float)a, (float)(b-1.0)))', x, y),
    )
    grad_y = binary_op(
        self,
        'a*b',
        grad_output,
        binary_op(self, 'pow(a, (float)b) * log(a);', x, y),
    )
    return unbroadcast(self, grad_x, x.shape), unbroadcast(self, grad_y, y.shape)
...
class Transpose(Function):
  def forward(self, x, order=(1,0)):
    ...
    return perm_axis(self, x, order)

  def backward(self, grad_output):
    return perm_axis(self, grad_output, np.argsort(self.order))
...
class Slice(Function):
  def forward(self, x, arg=None):
    self.save_for_backward(x.shape)
    return inner_slice(self, x, arg)

  def backward(self, grad_output):
    shape, = self.saved_tensors
    narg = [(0-p[0], grad_output.shape[i]+(shape[i]-p[1])) for i,p in enumerate(ctx.arg)]
    return inner_slice(self, grad_output, narg)
...
class Matmul(Function):
  def forward(self, input, weight):
    ...
    cnt = np.prod(input.shape[:-2]) if len(input.shape) > 2 else 1
    isize, msize, osize = i32(input.shape[-2]), i32(input.shape[-1]), i32(weight.shape[-1])
    ret = buffer_new(self, list(input.shape[:-2]) + [isize, osize])
    ...
    self.save_for_backward(input, weight, matmul, cnt)
  ...
  def backward(self, grad_output):
    input, weight, matmul, cnt = self.saved_tensors
    isize, msize, osize = i32(input.shape[-2]), i32(input.shape[-1]), i32(weight.shape[-1])

    grad_input = buffer_new(self, input.shape)
    grad_weight = buffer_new(self, weight.shape)
...
class Conv2D(Function):
  def forward(self, x, w, stride=1, groups=1):
    ...
    if cin * self.groups != cin_:
      raise Exception(
          f"Input Tensor shape {x.shape} does not match the shape of the weights {w.shape}. ({cin * self.groups} vs. {cin_})"
      )
    ...
    # output buffer
    ret = buffer_new(self, (bs, cout, oy, ox))
  ...
  def backward(self, grad_output):
    ...
    dx = buffer_new(self, (bs, cin_, iy, ix), zero=True)
    dw = buffer_new(self, (cout, cin, H, W))
    ...
    convw(self.cl_queue, [self.groups * rcout * cin, H, W], None, x.cl,
          grad_output.cl, dw.cl, *conv_args)
    convx(self.cl_queue, [bs, self.groups, cin], None, w.cl, grad_output.cl,
          dx.cl, *conv_args)
    return dx, dw
